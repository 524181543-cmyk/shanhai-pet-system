...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @keyframes qinglong-fly {
            0%, 100% { transform: translate(-50%, -50%) rotateZ(-5deg) translateY(0); }
            50% { transform: translate(-50%, -50%) rotateZ(5deg) translateY(-15px); }
        }
    ...
    <div class="container">
        ...
        <div id="adopt-panel" class="panel active">
            ...
            <div id="pet-section" style="display:none">
                <h2>第二步：选择你的神兽</h2>
                <p style="color: #636e72; margin-bottom: 20px;">每种神兽都有独特的属性和技能，选择你的命运伙伴吧！</p>
        ...
        <div id="points-panel" class="panel">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 20px;">
                ...
                <div id="points-history">
                    <h3 style="margin-bottom: 15px;">积分历史</h3>
    ...
    <script>
        ...
        const BEASTS = {
            1: {
                ...
                element: '土',
                rarity: '传说',
                description: '《山海经》记载：麒麟能活两千年，性情温和，不履生虫，不折生草。头上有角，角端有肉，设武备而不用。',
                color: '#ffd700',
                skills: ['祥瑞之兆', '仁兽护佑', '土灵庇护']
            },
            2: {
                name: '凤凰',
                icon: '🦅',
                element: '火',
                rarity: '传说',
                description: '《山海经》记载：丹穴之山有鸟焉，其状如鸡，五采而文，名曰凤凰。见则天下安宁，不死不灭，浴火重生。',
                color: '#ff6b6b',
                skills: ['涅槃重生', '烈焰焚天', '神火护体']
            },
            3: {
                name: '九尾狐',
                icon: '🦊',
                element: '幻',
                rarity: '史诗',
                description: '《山海经》记载：青丘之山有兽焉，其状如狐而九尾，其音如婴儿。通灵达意，千变万化，智慧超群。',
                color: '#ff9ff3',
                skills: ['幻化万千', '魅惑众生', '灵狐九变']
            },
            4: {
                name: '玄武',
                icon: '🐢',
                element: '水',
                rarity: '传说',
                description: '《山海经》记载：北方之神，龟蛇合体。主北方，司水。力大无穷，防御无双，千载寿元。',
                color: '#48dbfb',
                skills: ['玄武护盾', '北冥寒冰', '神龟延寿']
            },
            5: {
                name: '白虎',
                icon: '🐯',
                element: '金',
                rarity: '传说',
                description: '《山海经》记载：西方之神，主杀伐。威猛无匹，啸动山林。战力无双，百兽震惶。',
                color: '#ffffff',
                skills: ['白虎杀阵', '金戈铁马', '虎啸山林']
            },
            6: {
                name: '青龙',
                icon: '🐲',
                element: '木',
                rarity: '传说',
                description: '《山海经》记载：东方之神，主生机。腾云驾雾，呼风唤雨。护佑苍生，万物复苏。',
                color: '#54a0ff',
                skills: ['龙腾九天', '木灵复苏', '呼风唤雨']
            }
        };
                    ...
                    const petsHtml = data.data.map(pet => {
                        ...
                        return `
                            <div class="pet-card" onclick="adoptPet(${pet.id}, '${pet.name}')">
                                ...
                                <div class="pet-element">${beast.element}属性 · ${beast.rarity}</div>
                            </div>
                        `;
                    }).join('');
        ...
        async function adoptPet(petTypeId, petName) {
            ...
            const beast = BEASTS[petTypeId];
            const nickname = prompt('给你的' + petName + '起个名字吧：', petName);
        ...
        async function loadMyPet() {
            ...
            if (!userId) {
                document.getElementById('my-pet-content').innerHTML = `
                    <div class="message info">请先领养宠物</div>
                `;
                return;
            }
                ...
                if (data.success && data.data.length > 0) {
                    ...
                    document.getElementById('my-pet-content').innerHTML = `
                        <div class="pet-3d-container">
                            <div class="pet-3d-stage">
                                <div class="beast-glow ${beastClass}-glow"></div>
                                <div class="beast ${beastClass}" id="pet-beast">${pet.icon || beast.icon}</div>
                                <div class="particles">
                                    ${generateParticles(beast.color)}
                                </div>
                            </div>
                        </div>
                        
                        <div class="pet-info-card">
                            <h3>${pet.icon || beast.icon} ${pet.nickname}</h3>
                            <div class="beast-description">${beast.description}</div>
                            
                            <div class="pet-stats">
                                <div class="pet-stat">
                                    <div class="pet-stat-value">${pet.current_stage}</div>
                                    <div class="pet-stat-label">进化阶段</div>
                                </div>
                                <div class="pet-stat">
                                    <div class="pet-stat-value">${pet.total_points}</div>
                                    <div class="pet-stat-label">累计积分</div>
                                </div>
                                <div class="pet-stat">
                                    <div class="pet-stat-value">${beast.element}</div>
                                    <div class="pet-stat-label">元素属性</div>
                                </div>
                            </div>
                            ...
                            <div style="margin-top: 20px;">
                                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                    <span>🍖 饱食度</span>
                                    <span id="hunger-value">${pet.hunger}%</span>
                                </div>
                                <div class="hunger-bar">
                                    <div class="hunger-fill" id="hunger-fill" style="width: ${pet.hunger}%"></div>
                                </div>
                            </div>
                        </div>
                        
                        <div style="text-align: center; margin-top: 20px;">
                            <h3 style="margin-bottom: 15px;">喂养你的神兽</h3>
                            <p style="color: #636e72; margin-bottom: 15px;">消耗积分喂养神兽，提升饱食度</p>
                            <div>
                                <button class="feed-btn" onclick="feedPet(10)">🍎 喂养10积分</button>
                                <button class="feed-btn" onclick="feedPet(30)">🥩 喂养30积分</button>
                                <button class="feed-btn" onclick="feedPet(50)">🍗 喂养50积分</button>
                            </div>
                        </div>
                    `;
                } else {
                    document.getElementById('my-pet-content').innerHTML = `
                        <div class="message info">你还没有领养宠物，快去领养一只吧！</div>
                    `;
                }
            } catch (e) {
                document.getElementById('my-pet-content').innerHTML = `
        ...
        function generateParticles(color) {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                particles += `<div class="particle" style="left: ${left}%; animation-delay: ${delay}s; background: ${color};"></div>`;
            }
            return particles;
        }
                    ...
                    if (historyData.success) {
                        const historyHtml = historyData.data.map(record => `
                            <div class="leaderboard-item">
                                <div class="rank" style="background: ${record.points > 0 ? 'linear-gradient(135deg, #4caf50, #2e7d32)' : 'linear-gradient(135deg, #f44336, #c62828)'}">
                                    ${record.points > 0 ? '+' : ''}${record.points}
                                </div>
                                <div style="flex: 1">
                                    <div><strong>${record.reason}</strong></div>
                                    <div style="color: #636e72; font-size: 0.85em">${record.category} · ${new Date(record.created_at).toLocaleDateString()}</div>
                                </div>
                            </div>
                        `).join('');
                ...
                if (data.success) {
                    if (data.data.length === 0) {
                        document.getElementById('leaderboard-list').innerHTML = `
                            <div class="message info">暂无排行数据</div>
                        `;
                        return;
                    }
                        ...
                        return `
                            <div class="leaderboard-item">
                                ...
                                <div style="flex: 1">
                                    ...
                                    <div style="color: #636e72; font-size: 0.85em">@${user.username}</div>
                                </div>
...
async def adopt_pet(user_id: int, pet: PetAdopt):
    ...
    try:
        ...
        result.data[0]['icon'] = pet_icons.get(pet.pet_type_id, '🐉')
